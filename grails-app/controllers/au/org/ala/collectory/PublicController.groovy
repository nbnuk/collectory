...
class PublicController {
    ...
    def show = {
        ...
        } else if (params.id instanceof String && params.id.startsWith('co')) {
            forward(action: 'showCollection', params: params)
        } else {
        ...
        results.each {
            ...
            if (it) {
                ...
                if (it.collections) {
                    ...
                    if (showAll ) {
                        relevantCollections = it.collections
                    } else {
                    ...
                    if (relevantCollections){
                        ...
                        loc.properties = [
                                ...
                                collections: relevantCollections.collect { [uid: it.uid, name:it.name, url:request.getContextPath() + "/public/show/" + it.uid] }
                        ]
    ...
    String contructHTML(entity, childEntities, baseUrl){
        ...
        markup.span {
            ...
            span(collectionsStr)
            ul {
                childEntities.each { childEntity ->
                    li {
                        a (href: baseUrl + childEntity.uid,  childEntity.name)
                    }
                }
            }
        }
